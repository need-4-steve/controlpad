package com.controlpad.payman_processor.datasource;
...
public class SqlSessionUtil {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(SqlSessionUtil.class);
    ...
    public SqlSessionUtil() {
        ...
        try {
            ...
            paymanDatasource = new DBDataSource(new Gson().fromJson(new FileReader(sqlConfigFile), SqlConfig.class));
        } catch (Exception e) {
            logger.error("Failed to get main db connection", e);
            //TODO notify
    ...
    public boolean existsDatasourceForClient(String clientId) {
        return (clientDatasourceMap.get(clientId) != null);
    }

    public void addClientDatasource(List<ControlPadClient> clients) {
        for (ControlPadClient client: clients) {
            addClientDatasource(client);
        }
    }
    ...
    private SqlSession openSessionFromDataSource(DBDataSource dbDataSource, ExecutorType execType, TransactionIsolationLevel level, boolean autoCommit) {
        ...
        try {
            ...
            logger.error("Failed to open sql session", e);
            //TODO notify
