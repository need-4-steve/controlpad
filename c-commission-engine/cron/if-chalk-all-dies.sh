...
CHALK_SIM1=$(curl -sS --connect-timeout 30 http://localhost:8081/ -o - | wc -c);
CHALK_SIM2=$(curl -sS --connect-timeout 30 http://localhost:8082/ -o - | wc -c);

if [ $CHALK_LIVE -ne "146" ] || 
   [ $CHALK_SIM1 -ne "146" ] || 
   [ $CHALK_SIM2 -ne "146" ]; then
	echo "* RESTARTING ALL CHALK API!"
	/etc/ceapi/stop.sh
	/etc/ceapi/start.sh
