...
CezJson::CezJson()
{
    ...
    m_StatusMap[413] = "Request Entity Too Large";
    m_StatusMap[414] = "Request-URI Too Long";
    m_StatusMap[415] = "Unsupported Media Type";
    m_StatusMap[416] = "Requested Range Not Satisfiable";
...
////////////////////////////////////////
// Set the string for the Json object //
////////////////////////////////////////
const char *CezJson::SetJson(int status, const char *json)
{
    std::stringstream httpstr;

#ifdef COMPILE_HTTP_HEADERS
    // Http header //
    httpstr << "HTTP/1.1 " << status << " " << m_StatusMap[status] << "\r\n";
    if (m_Origin.size() > 0)
    {
        httpstr << "Access-Control-Allow-Origin: " << m_Origin << "\r\n";
        httpstr << "Access-Control-Allow-Credentials: true\r\n";
    }
    httpstr << "Content-Type: application/vnd.api+json\r\n\r\n";
#endif

    if (strlen(json) == 0)
        httpstr << "{\"success\":{\"status\":\"" << status << "\"}}";
    else
        httpstr << "{\"success\":{\"status\":\"" << status << "\"}," << &json[1] << "}"; // Skip past the , for now //
        //    httpstr << "{" << &json[1] << "}"; // Skip past the , for now //
    m_Json = httpstr.str();
...
////////////////////////////////////////
// An easier way of constructing Json //
////////////////////////////////////////
//const char *CezJson::SetJson(int status, std::stringstream& json_str)
const char *CezJson::SetJson(int status, basic_ostream<char> &json_str)
{
    //std::string json;
    //json = json_str.str();
    stringstream json;
    json << json_str.rdbuf();
    return SetJson(status, json.str().c_str());
}

////////////////////////////////////////////////////////////////////
// Set the Json error info and return a string of the json object //
////////////////////////////////////////////////////////////////////
const char *CezJson::SetError(int status, const char *source, const char *title, const char *detail)
{ 
    std::stringstream httpstr;
    ...
    // Json object //
    httpstr << "{\"errors\":{\"status\":\"" << status << "\",\"source\":\"" << source << "\",\"title\":\"" << title << "\",\"detail\":\"" << detail << "\"}}";
    m_Json = httpstr.str();
    return m_Json.c_str();
}
...
/////////////////////////////////////
// Needed for stupid Angular stuff //
/////////////////////////////////////
const char *CezJson::SetCORSResp(const char *origin, const char *request_methods, const char *access_headers)
{
    m_Json.clear();
    std::stringstream httpstr;
    ...
    // Build the allow access control headers //
    httpstr << "HTTP/1.1 200 OK\r\n";
    httpstr << "Access-Control-Allow-Origin: *\r\n"; // << origin << "\r\n";
    httpstr << "Access-Control-Allow-Methods: " << request_methods << "\r\n";
    httpstr << "Access-Control-Allow-Headers: " << access_headers << "\r\n";
    httpstr << "Access-Control-Allow-Credentials: true\r\n";
    httpstr << "Content-Type: text/plain\r\n\r\n";
    httpstr << "{\"method\":\"OPTIONS\"}\r\n\r\n";
    m_Json = httpstr.str();
    return m_Json.c_str();
}
...
//////////////////////////
bool CezJson::SetOrigin(string origin)
{
    m_Origin = origin;
    return true;
}
