...
class MemoryCache
{
     ...
     */
    protected function generateID($file=null)
    {
        $id = ftok(is_null($file) ? __FILE__ : $file, "b");
        return $id;
    }
    /**
    ...
    {
        if (is_null($id)) $id=is_null($this->id) ? ftok(__FILE__, "1") : $this->id;
        $size = shmop_size(shmop_open($id, "w", 0, 0));
        $data = shmop_read(shmop_open($id, "w", 0, 0), 0, $size);
        return $data;
    }
    /**
    ...
    {
        if (!is_null($id) || !is_null($this->id)) shmop_delete(is_null($id) ? shmop_open($this->id, "w", 0, 0) : shmop_open($id, "w", 0, 0));
        else delete(shmop_open(ftok(__FILE__, "1"), "w", 0, 0));
    }
    /**
    ...
    {
        if (!is_null($this->shmid)) shmop_close($this->shmid);
    }
}