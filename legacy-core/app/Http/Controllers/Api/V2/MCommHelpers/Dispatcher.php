...
use App\Http\Controllers\Api\V2\MCommHelpers\Exceptions\HttpException;
use App\Http\Controllers\Api\V2\MCommHelpers\Exceptions\ClientException;
use App\Http\Controllers\Api\V2\MCommHelpers\Exceptions\ServerException;
use App\Http\Controllers\Api\V2\MCommHelpers\Exceptions\RedirectException;
use App\Http\Controllers\Api\V2\MCommHelpers\Exceptions\MiscellaneousException;
...
class Dispatcher
{
     ...
     * @throws MiscellaneousException
     * @throws RedirectException
     * @throws ServerException
     * @throws \GuzzleHttp\Exception\GuzzleException
     */
    public function request($type, $url, $headers = [], $body = [])
    {
        try {
            $this->data = [];
            $client = new GuzzleClient();
            $this->prepareHeaders($headers);
            $this->setDataByHeaderType($type, $body, $url);
     ...
     * @param $url
     */
    public function setDataByHeaderType($type, $body, $url = null)
    {
        ...
        if ($method == 'GET') {
            ...
            $this->data['query'] = $body ? array_merge($body, $query) : $query;
        } else {
            //TODO - handle multipart as well
            if ($headers['Content-Type'] === 'application/json' || $headers['Content-Type']==='application/javascript') {
                $this->data['json'] = $body;
            } else {
                $this->data['form_params'] = $body;
            }
        }
    }
    /**
    ...
    protected function checkStatusCode($code, $message = '', $stackTrace = '', $clientResponse = null)
    {
        ...
        //Status code 1**
        if ($this->isMiscellaneous($code)){
            throw new MiscellaneousException($code, $message, $stackTrace, $clientResponse);
        }
    }
    /**
     ...
     * @return bool
     */
    public function isMiscellaneous($status)
    {
        return $status < 200;
    }
    /**
