...
class AutoshipSubReminderMailer
{
    ...
    {
        $reminderEmail = CustomEmail::where('title', 'autoship_reminder')->first();
        if (!$reminderEmail->send_email) {
            return;
        }
        ...
        $varables = $this->emailService->emailVar($reminderEmail->title);
        $request = [
            'buyer_first_name' => $subscription['buyer']['first_name'],
        ...
        $request['subscription_next_billing_date'] = Carbon::parse($subscription['next_billing_at'], 'UTC')->setTimezone($timezone)->format('M d Y');

        $body = $this->textService->parseText($request, $varables, $reminderEmail->body);
        $reminderEmail->subject = $this->textService->parseText($request, $varables, $reminderEmail->subject);
        $boundData = [
            'body' => $body
        ];
        ...
        Mail::send(
            'emails.standard',
            $boundData,
            function ($message) use ($subscription, $fromEmail, $fromName, $reminderEmail, $buyerEmail, $buyerName) {
